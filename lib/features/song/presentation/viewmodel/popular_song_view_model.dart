...
final popularSongViewmodelProvider = StateNotifierProvider<SongViewmodel, SongState>(
      (ref) {
...
class SongViewmodel extends StateNotifier<SongState> {
  ...
  getPopularSongs() async {
    ...
    if (!hasReachedMax) {
      var data = await songUseCase.getPopularSongs(page);
      data.fold(
            (l) {
